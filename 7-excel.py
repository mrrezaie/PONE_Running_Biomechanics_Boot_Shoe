...
grf = pd.read_pickle('collate\\grf.zip')
joint = pd.read_pickle('collate\\jointc.zip')
velContra = pd.read_pickle('collate\\velc.zip')
bodyContra = pd.read_pickle('collate\\bodyc.zip')
moment = pd.read_pickle('collate\\moment.zip')
power = pd.read_pickle('collate\\power.zip')
TD = pd.read_pickle('collate\\TD.zip')
demo = pd.read_csv('demographics.csv', index_col=0)
g = 9.806
...
grf2.to_excel('collate\\data_grf.xlsx', sheet_name=grfLabel[0])
writer = pd.ExcelWriter('collate\\data_grf.xlsx', mode='a', if_sheet_exists='replace')
for i in grfLabel:
	grf2=pd.DataFrame(index=range(101), columns=[level1, level2, level3])
	for j in subj:
		for k in ['boot', 'shoe']:
...
power2.to_excel('collate\\data_power.xlsx', sheet_name=powerLabel[0])
writer = pd.ExcelWriter('collate\\data_power.xlsx', mode='a', if_sheet_exists='replace')
for i in powerLabel:
	power2=pd.DataFrame(index=range(101), columns=[level1, level2, level3])
	for j in subj:
		for k in ['boot', 'shoe']:
...
moment2.to_excel('collate\\data_moment.xlsx', sheet_name=momentLabel[0])
writer = pd.ExcelWriter('collate\\data_moment.xlsx', mode='a', if_sheet_exists='replace')
...
joint2.to_excel('collate\\data_angle.xlsx', sheet_name=jointLabel[0])
writer = pd.ExcelWriter('collate\\data_angle.xlsx', mode='a', if_sheet_exists='replace')
for i in jointLabel:
	joint2=pd.DataFrame(index=range(101), columns=[level1, level2, level3])
	for j in subj:
		for k in ['boot', 'shoe']:
